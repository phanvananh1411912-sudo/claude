...
lessons = json.load(open(f'{SP}/parsed_hsk3.json', encoding='utf-8'))['hsk3']

enr = {}
for f in glob.glob(f'{SP}/enriched/sent_hsk3_*.out.json'):
...
for L in lessons:
    ...
    if len(es) != len(L['sentences']): errors.append(f'COUNT {L["no"]}'); continue
    ...
    for src, e in zip(L['sentences'], es):
        ...
        cau.append((bai, e['idx'], src['han'], py, vi, '; '.join(tags)))
    for g in gnames:
...
out = f'{SP}/HSK4NEXUS-HSK3-40bai.xlsx'
